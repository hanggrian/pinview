...
public class PinView extends LinearLayout {
  ...
  public int getSelection() {
    return map(input -> input.isFocused(), input -> indexOfChild(input), -1);
  }
  ...
  public void setText(@NonNull CharSequence text) {
    ...
    forEachIndexed(pair -> {
      if (pair.second < pins.length) {
        Preconditions.checkArgument(Character.isDigit(pins[pair.second]), "Text should be digits");
  ...
  private void applyGap() {
    if (pinGap > -1 && getCount() > 1) {
      forEach(view -> ((MarginLayoutParams) view.getLayoutParams()).setMargins(
          pinGap / 2,
          0,
          pinGap / 2,
          0
